...
u8  tbl_mac [] = {0x23, 0x17, 0x16, 0x18, 0x11, 0x19};
u8	tbl_adv [42] =
	{	0x00, 37,
		...
		19, 0x09, 	  
		'D','E','M','O','_','H','I','D','_','0','1','1','1',' ',' ',' ',' ',' ',// must be is 18 byte
...
static inline void hw_scan(){
	...
	static u8 consumerKey_en,boardKey_en;
	if(button_value == 0){				///release button send release key
		if(consumerKey_en){
			consumerKey_en = 0;
			flag_has_new_event_tmp|=SEND_C_DATA;
...
//	else
	{
		if(button_value&0x01){					///gp10 gnd  send number 2
			flag_has_new_event_tmp|=SEND_K_DATA;
			reportKeyIn[2]= 0x1F;
			boardKey_en =1;
		}
		else if(button_value&0x02){				///gp10 vdd  send v+
			consumerKey_en =1;
			flag_has_new_event_tmp|=SEND_C_DATA;
			reportConsumerControlIn[0]=0xe9;
		}
